  ...
  <div class="wrap">
    ...
    <mt-index-list :height="indexListHeight">
      <mt-index-section v-for="{key, values} in characters" :index="key" v-show="values.length > 0">
        <mt-cell v-for="employee in values" :title="employee.tblCompany_ComFullName" @click.native="check(employee)">
        </mt-cell>
      </mt-index-section>
     </mt-index-list>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData () {
        ...
        this.popupSelect({selectName: 'SelectCompanySales', pageSize: 999}).then((data) => {
          Indicator.close()
          // console.log(data)
          vm.characters.forEach(({values}) => { values.splice(0, values.length) })
          data.forEach(employee => {
            let index = employee.tblCompany_ComFullNamePYM.charCodeAt(0) - 97
          ...
          vm.characters.forEach(ch => ch.values.sort((a, b) => a.tblCompany_ComFullName.localeCompare(b.tblCompany_ComFullName)))
        })
      },
      initData () {
        for (let i = 0; i < 26; i++) {
