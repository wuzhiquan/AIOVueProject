      ...
      <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" ref="loadmore" :autoFill="false">
        <div class="list_item" v-for='(item, index) in dataList' @click="detailData(item)">
          ...
          <p class="SourceInfo_p">
            <span class="SourceInfo" v-if="item.SourceInfo">{{item.SourceInfo}}</span>
            <span class="Status" v-if="item.Status">{{item.Status}}</span>
            <span class="ClientType" v-if="item.ClientType">{{item.ClientType}}</span>
          </p>
          <p class="createBy" v-if="item.createBy2">联系人：{{item.createBy2}}</p>
          <p class="Emergency_p">
            <span class="left" v-if="item.Emergency">紧急程度：<span class="Emergency">{{item.Emergency}}</span></span>
            <span class="right" v-if="item.LastContractTime">上次联系：<span class="LastContractTime">{{item.LastContractTime.substring(0,10)}}</span></span>
          </p>
          <!-- <em class="phone"></em> -->
        </div>
      </mt-loadmore>
