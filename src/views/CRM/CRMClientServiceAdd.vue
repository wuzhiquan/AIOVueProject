...
<div class="page_wrap">
  <!-- 顶部 -->
  <mt-header title="添加服务记录" fixed>
    <div slot="left">
      <mt-button icon="back" @click="$router.go(-1)">返回</mt-button>
    </div>
  </mt-header>
  <!-- 内容 -->
  <div class="page_cont CRMServiceAdd_page">
    <!-- 主表内容 -->
...
export default {
  ...
  methods: {
    ...
    addPreList () {
      ...
      this.addPre({tableName: 'CRMClientService', parentTableName: 'CRMClientInfo'}).then((obj) => {
        Indicator.close()
        let vm = this
    ...
    'save': function () {
      ...
      this.add({tableName: 'CRMClientService', values}).then(({keyId, tableName}) => {
        Indicator.close()
        Toast({message: '提交成功', duration: 1000})
        setTimeout(() => {
          this.$router.replace({name: 'CRMClientServiceList'})
        }, 1000)
      }).catch(({message}) => {
        Indicator.close()
    ...
    '$route' (to, from) {
      ...
      if (from.name === 'popupEmployee') {
        if (to.params.checked) {
          ...
          vm.$data.EmployeeName = checkedEmp.tblEmployee_EmpFullName
        }
...
<style>
  .CRMServiceAdd_page .mint-cell-wrapper,
  .CRMServiceAdd_page .mint-cell:last-child{
    background-image:none;
    padding: 0;
  }
  .CRMServiceAdd_page input{
    color:#98979d;
  }
  .CRMServiceAdd_page input::-webkit-input-placeholder {
    color:#ddd;
  }
  .CRMServiceAdd_page input::-moz-placeholder{
    color:#ddd;
  }
  .CRMServiceAdd_page input::-moz-placeholder{
    color:#ddd;
  }
  .CRMServiceAdd_page input:-ms-input-placeholder{
    color:#ddd;
  }
</style>
