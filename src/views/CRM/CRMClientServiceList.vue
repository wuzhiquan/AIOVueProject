      ...
      <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" ref="loadmore" :autoFill="false">
        <div class="list_item" v-for='(item, index) in datalist' @click="detailData(item)">
          <p class="EmpFullName">
            ...
            <em v-if="item.EmpFullName&&item.ServeState"></em>
            <span>{{item.ServeState}}</span>
            <em v-if="(item.ServeState&&item.ServeType)||(item.EmpFullName&&item.ServeType)"></em>
            <span>{{item.ServeType}}</span>
            <span style="float:right">{{item.BillDate.substring(0,10)}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      'detailData': function (item) {
        console.log(item)
        this.$router.push({
          name: 'CRMClientServiceDetail',
          params: {keyId: item.keyId}
        })
      },
      'add': function () {
        this.$router.push({name: 'CRMClientServiceAdd'})
      },
      // 下拉处理
      loadTop (id) {
        this.pageNo = 1
